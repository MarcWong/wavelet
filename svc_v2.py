...
def generate_label(X):
    ...
    for i in range(0, data_size):
        ...
        if X[i][0] ** 2 + X[i][1] ** 2 > OUTLIER_DEVIATION:  # TODO 这里改了。改的原因是，一阶的判断标准应该和均匀分布搭配，二阶以上的的标准应该和正态分布搭配。如果要以其他标准来判断outlier，就改这里
            Y[i] = OUTLIER_LABEL
        else:
            Y[i] = NORMAL_LABEL
...
X_test_all = np.random.uniform(-1, 1, (test_size, 2))
