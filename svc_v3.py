...
def generate_label(X):
    ...
    #     else:
    #         Y[i] = NORMAL_LABEL
    outlier_ratio = 0.2 #数据中outlier占多少比例
    outlier_shift = 0.2 # 数据中outlier偏移多少
    outlier_start_index = 0
    outlier_end_index = int(outlier_ratio * data_size)
    for i in range(0, data_size):
        if i >= outlier_start_index and i < outlier_end_index:
            Y[i] = OUTLIER_LABEL
            X[i, :] += 0.1
            X[i, :] *= 2
        else:
